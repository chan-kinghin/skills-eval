...
14. [CLI 参考](#14-cli-参考)
15. [配置（`config.yaml`）](#15-配置)
16. [模型目录](#16-模型目录)
17. [支持的输入文件](#17-支持的输入文件)
18. [比较器](#18-比较器)
19. [结果与输出](#19-结果与输出)
20. [示例演练](#20-示例演练)
21. [故障排除 / 常见问题](#21-故障排除--常见问题)
...
**比较器**是用于检查模型输出是否与预期结果匹配的策略。SkillEval 内置了 6 种比较器（参见[比较器](#18-比较器)）。
...
将模型需要处理的文件放在 `input/` 目录中。SkillEval 支持多种格式（参见[支持的输入文件](#17-支持的输入文件)）。所有输入文件会被拼接并作为提示词的一部分发送。
