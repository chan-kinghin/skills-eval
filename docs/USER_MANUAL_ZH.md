...
9. [CLI 参考](#9-cli-参考)
10. [配置（`config.yaml`）](#10-配置)
11. [模型目录](#11-模型目录)
12. [支持的输入文件](#12-支持的输入文件)
13. [比较器](#13-比较器)
14. [结果与输出](#14-结果与输出)
15. [示例演练](#15-示例演练)
16. [故障排除 / 常见问题](#16-故障排除--常见问题)
...
**比较器**是用于检查模型输出是否与预期结果匹配的策略。SkillEval 内置了 6 种比较器（参见[比较器](#13-比较器)）。
...
将模型需要处理的文件放在 `input/` 目录中。SkillEval 支持多种格式（参见[支持的输入文件](#12-支持的输入文件)）。所有输入文件会被拼接并作为提示词的一部分发送。
...
如果 API 调用总数超过 100 次，SkillEval 会在执行前请求确认。使用 `--confirm` 跳过此提示：
...
| `--parallel` | 否 | `20` | 最大并发 API 调用数 |
| `--catalog` | 否 | 自动检测 | 模型目录 YAML 文件路径 |
| `--confirm` | 否 | `false` | 跳过大规模运行（>100 次 API 调用）的确认 |
...
SkillEval 内置了一个涵盖 3 个供应商、10 个模型的默认模型目录：
...
运行 `skilleval catalog` 验证哪些模型显示"Ready"。
